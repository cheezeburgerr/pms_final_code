...
import PropertiesBubble from './PropertiesBubble';
...
export default function Configurator({ auth, product }) {
    ...
    }, [selectedObject, addToModel]);


    useEffect(() => {
      DesignerFunctions.displayObjects(canvas);
    ...
    return (
        <>
            {/* <label id="zoom">
    <input type="range" min="1" max="100"  step="0.1"/>
</label> */}
            ...
            <div className="z-[99999] p-2 pl-8 lg:px-24 dark:bg-aqua space-x-1 bg-aqua flex justify-between gap-3 items-center">
                {/* <p>Configurator</p> */}

                <div className="grid grid-cols-5 gap-6 w-1/4 items-center justify-items-center">
                ...
                <Dropdown className='z-50'>
                    <Dropdown.Trigger>
                        <span className="inline-flex rounded-md">
                            ...
                            >
                                <span><IconDeviceFloppy color='black' /></span>
                            </button>
                        </span>
                    </Dropdown.Trigger>
            ...
            <div className='lg:flex flex-row-reverse justify-start gap-4 h-full'>
                <div className="flex">
                ...
                <div className='z-50 w-[500px] relative bg-gray-900'>
                    <div className="lg:ms-4 absolute inset-x-0 lg:right-auto left-0 lg:inset-y-0  bottom-0  flex flex-col-reverse lg:flex-row  items-center justify-center ">

                        <div className='dark:text-gray-100 w-full lg:w-auto flex lg:flex-col lg:gap-16 justify-center items-center dark:bg-zinc-800 bg-gray-100 rounded-lg p-3 h-full'>
                            ...
                            <div className='text-center flex flex-col items-center w-full cursor-pointer' onClick={() => toggleTab('5')}>
                                <IconPhoto />
                                <small>Images</small>
                            </div>
                            <div className='text-center flex flex-col items-center w-full cursor-pointer' onClick={() => toggleTab('6')}>
                                <IconStack />
                                <small>Layers</small>
                            </div>

                        </div>
                        <div className={`${showProperties ? 'block' : 'hidden'} h-full bg-gray-100 dark:bg-zinc-800 dark:text-gray-100 p-6 rounded-lg lg:rounded-none shadow-lg mt-0 border-l border-zinc-300 dark:border-zinc-700 w-full lg:min-w-[350px] lg:max-w-[250px]`}>
                            <div className={`${activeTab === '1' ? 'block' : 'hidden'}`}>
                                <p className="font-bold">Cloth Colors</p>
                                <small className='mb-2'>Adjust colors in each part of the model</small>
                                <div id="colorPickerContainer" className='overflow-x-auto flex lg:block'></div>
                            ...
                            <div className={`${activeTab === '2' ? 'block' : 'hidden'} `}>
                                ...
                                <div id="shapes"></div>
                                <div className='flex gap-2 lg:grid grid-cols-2'>
                                ...
                                <div className="flex gap-2 lg:grid grid-cols-2">
                                    ...
                                    <button onClick={() => DesignerFunctions.addCircle(canvas, setAddToModel)} id="circle-button" className='p-2 cursor-pointer'><svg width="25" height="25" xmlns="http://www.w3.org/2000/svg" title='Circle Tool'>
                                        <circle r="12" cx="12" cy="12" fill='white' stroke='black' strokeWidth={1} />
                                    </svg></button>
                                </div>
                            ...
                            <div className={`${activeTab === '5' ? 'block' : 'hidden'}  lg:h-[calc(100vh/2)] overflow-y-auto`}>

                                <p className='mb-2'>Images</p>
                                <div className='flex gap-2 w-full'>
                                <TextInput type="file" id='upload-image' onChange={() => DesignerFunctions.addImage(canvas, setAddToModel)}  className='w-full' />
                                    {/* <PrimaryButton id="add-text-btn" onClick={() => DesignerFunctions.addText(canvas, setAddToModel)}>Add</PrimaryButton> */}
                                </div>
                            ...
                            <div className={`${activeTab === '6' ? 'block' : 'hidden'}  lg:h-[calc(100vh/2)] overflow-y-auto`}>
            ...
            <div className='p-1'>
                <div className={` transition-all duration-300 h-full fixed inset-y-0 flex flex-col gap-4 items-start justify-center right-0 top-0 p-2`}>
                    ...
                    <div className={` ${isExpanded ? 'translate-x-0' : 'translate-x-[60px]'} transition-all dark:text-gray-100 flex flex-row items-center justify-center`}>
                        ...
                        <div className="flex flex-col gap-4 items-center bg-gray-100 dark:bg-zinc-800 p-1 rounded-lg">
                            ...
                            <div className="p-3">
                                <div onClick={() => setShowAvatarPicker(!showAvatarPicker)} className='h-5 w-5 border-1 cursor-pointer' style={{ backgroundColor: avatarColor }} title='Avatar Color'></div>
                            </div>
                            {showAvatarPicker && (
                                <>
                                    <div className="absolute -left-[250px]"><ChromePicker color={avatarColor} onChange={(color) => changeAvatarColor(color, setAvatarColor)} /></div>
                                </>
                            )}
                        </div>
                    ...
                    <div className={` ${isExpanded2 ? 'translate-x-0' : 'translate-x-[60px]'} transition-all dark:text-gray-100 flex flex-row items-center justify-center`}>
                        ...
                        <div className="flex flex-col gap-4 items-center bg-gray-100 dark:bg-zinc-800 p-2 rounded-lg">
                            ...
                            <input type="file" id='upload-image2' onChange={() => DesignerFunctions.addImage(canvas, setAddToModel)} ref={uploadRef} className='hidden' />
